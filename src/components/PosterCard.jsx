...
export default function PosterCard({ item, onFavoriteChange }) {
  ...
  return (
    <div
      className={`group relative block min-w-40 overflow-hidden rounded border border-white/10 bg-panel ${
        isChannel ? 'aspect-[16/10]' : 'aspect-[2/3]'
      }`}
    >
      <Link to={mediaLink(item)} className="card-focus absolute inset-0">
        {imageOk ? (
          <img
            src={item.posterUrl}
            alt={title}
      ...
      <FavoriteButton
        ...
        onChange={(next) => onFavoriteChange?.(item, next)}
        size="small"
        className="absolute right-1.5 top-1.5 z-10 shadow-lg ring-1 ring-black/20"
      />
    </div>
  );
}
